    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div>
        <label className="block text-[12px] font-semibold text-[#374151] mb-3">
          Areas of Specialisation <span className="text-[#7C3AED]">*</span>
        </label>
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
          {specialisms.map((spec: any) => {
            ...
            return (
              ...
              >
                <div className={`w-2 h-2 rounded-full transition-colors ${isSelected ? 'bg-[#7C3AED]' : 'bg-[#D1D5DB]'}`} />
                <span className={`text-[13px] font-medium flex-1 ${isSelected ? 'text-[#7C3AED]' : 'text-[#374151]'}`}>
                  {spec.displayName}
                </span>
      ...
      <div>
        <label className="block text-[12px] font-semibold text-[#374151] mb-3">
          Languages Spoken <span className="text-[#7C3AED]">*</span>
        </label>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-medium text-[#7C3AED] hover:bg-pink-50 transition-colors"
          >
        ...
        <div className="space-y-3">
          ...
          {form.education.length === 0 && (
            <button
              type="button"
              onClick={addEducation}
              className="w-full py-8 rounded-xl border-2 border-dashed border-[#E5E7EB] text-[13px] text-[#9CA3AF] hover:border-[#7C3AED] hover:text-[#7C3AED] transition-all"
            >
...
export function ConsultationStep({ form, updateForm, errors }: any) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div>
        <label className="block text-[12px] font-semibold text-[#374151] mb-3">
          Consultation Fees (NGN) <span className="text-[#7C3AED]">*</span>
        </label>
        <div className="space-y-3">
          {feeTypes.map(fee => {
            const Icon = fee.icon;
              ...
              <div key={fee.key} className="flex items-center gap-4 p-4 bg-[#F9FAFB] rounded-xl border border-[#F3F4F6] hover:border-[#E5E7EB] transition-all">
                ...
                <div className="flex items-center gap-2">
                  {isProbono ? (
                    ...
                    >
                      <div className={`w-4 h-4 rounded border flex items-center justify-center transition-all ${form.fees.probono ? 'bg-[#7C3AED] border-[#7C3AED]' : 'border-[#D1D5DB]'
                        }`}>
      ...
      <div>
        <label className="block text-[12px] font-semibold text-[#374151] mb-3">
          Typical Response Time <span className="text-[#7C3AED]">*</span>
        </label>
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="space-y-4">
        ...
        {REQUIRED_DOCUMENTS.map((doc) => {
          ...
          return (
            <div key={doc.id} className="border border-[#E5E7EB] rounded-xl p-4 hover:border-[#7C3AED]/30 transition-all">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <div className="flex items-center gap-2">
                    <p className="text-[14px] font-semibold text-[#111827]">{doc.label}</p>
                    {doc.required && (
                      <span className="text-[10px] font-semibold text-[#7C3AED] bg-pink-50 px-2 py-0.5 rounded-full">Required</span>
                    )}
                  </div>
              ...
              {isUploaded && uploadedDoc ? (
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    {uploadedDoc.fileUrl && (
                      <a
                        ...
                        className="p-1.5 rounded-lg text-[#7C3AED] hover:bg-pink-50 transition-colors"
                      >
                ...
                >
                  <div className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-all
                    ${error ? 'border-red-300 bg-red-50/30' : 'border-[#E5E7EB] hover:border-[#7C3AED] hover:bg-pink-50/20'}`}
                  >
              ...
              {isUploading && (
                <div className="mt-3">
                  <div className="h-1.5 bg-[#F3F4F6] rounded-full overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-[#7C3AED] to-[#5B21B6] transition-all duration-300 rounded-full"
            ...
            <div className="text-[13px]">
              ...
              <div className="flex flex-wrap gap-1.5">
                {form.languages.map((lang: string) => (
                  <span key={lang} className="text-[12px] bg-pink-50 text-[#7C3AED] px-2 py-0.5 rounded-full">
                    {lang}
                  </span>
                ))}
              </div>
