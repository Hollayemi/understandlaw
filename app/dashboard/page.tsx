...
const getTagColor = (tag: string) => {
  const colors: Record<string, string> = {
    "Police Rights": "#9B2E3D",
    "Tenancy Law": "#9B2E3D",
    "Employment Law": "#82212D",
    "Business Law": "#9B2E3D",
    "Family Law": "#9B2E3D",
    "Contract Law": "#82212D",
  };
  return colors[tag] || "#9B2E3D";
};
...
function DailyQuiz({
  ...
  return (
    <div className="bg-gradient-to-br from-[#111827] to-[#1E3A5F] rounded-2xl p-5 border border-white/8 relative overflow-hidden">
      <div className="absolute -top-8 -right-8 w-32 h-32 rounded-full bg-maroon-500/15 blur-3xl pointer-events-none" />

      <div className="flex items-center gap-2 mb-3">
        <Zap size={14} className="text-maroon-400" />
        <span className="text-[11px] font-bold uppercase tracking-widest text-maroon-400">Daily Challenge</span>
        <span className="ml-auto text-[11px] bg-maroon-400/15 text-maroon-400 font-semibold px-2 py-0.5 rounded-full">+{challenge.xpReward} XP</span>
      </div>
      ...
      <div className="flex flex-col gap-2">
        {challenge.options?.map((opt: string, i: number) => {
          ...
          if (revealed || completed) {
            ...
            cls = "border-maroon-500 bg-maroon-500/10 text-maroon-400";
          }
          return (
            <button
              key={opt}
      ...
      {selectedOption !== null && !revealed && !completed && (
        <button
          ...
          className="mt-3 w-full py-2.5 rounded-xl text-xs font-bold text-white bg-maroon-500 hover:bg-maroon-600 transition-colors"
        >
          Submit Answer
        </button>
      )}
...
function XPBar({ xpTotal, xpLevel }: { xpTotal: number; xpLevel: number }) {
  ...
  return (
    <div className="hidden md:flex items-center gap-3">
      <div className="w-8 h-8 rounded-lg bg-maroon-500/15 border border-maroon-500/20 flex items-center justify-center flex-shrink-0">
        <span className="text-sm font-bold text-maroon-500">{xpLevel}</span>
      </div>
      <div className="flex-1">
        ...
        <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
          <div className="h-1.5 rounded-full transition-all duration-700" style={{ width: `${pct}%`, background: "linear-gradient(90deg, #9B2E3D, #82212D)" }} />
        </div>
      </div>
    </div>
  );
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`w-8 h-8 rounded-full bg-white border border-gray-200 flex items-center justify-center transition-colors shadow-sm ${currentIndex === 0 ? "opacity-40 cursor-not-allowed" : "hover:border-maroon-500"
              }`}
          ...
          <div className="flex gap-1.5 px-2">
            {Array.from({ length: totalPages }).map((_, idx) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all ${idx === currentPage ? "w-6 bg-maroon-500" : "w-1.5 bg-gray-300 hover:bg-gray-400"
                  }`}
          ...
          <button
            ...
            className={`w-8 h-8 rounded-full bg-white border border-gray-200 flex items-center justify-center transition-colors shadow-sm ${currentIndex + itemsPerPage >= items.length ? "opacity-40 cursor-not-allowed" : "hover:border-maroon-500"
              }`}
            ...
            <div className="flex-1 p-5">
              <div className="flex items-start justify-between gap-3 mb-3">
                <div>
                  ...
                  <h3 className="font-bold text-gray-900 text-sm mt-0.5 leading-snug group-hover:text-maroon-500 transition-colors">
                    {item.title}
                  </h3>
                </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] bg-maroon-50 text-maroon-700 font-semibold px-2 py-0.5 rounded-full border border-maroon-100 flex items-center gap-1">
...
export default function UserDashboardOverview() {
  ...
  // Stats configuration
  const STATS = [
    { icon: BookOpen, color: "#9B2E3D", bg: "#fff0f2", value: stats?.topicsCompletedCount ?? 0, label: "Topics Read" },
    { icon: Flame, color: "#9B2E3D", bg: "#fff0f2", value: stats?.streakDays ?? 0, label: "Day Streak" },
    { icon: Trophy, color: "#9B2E3D", bg: "#fff0f2", value: stats?.certificatesCount ?? 0, label: "Certificates" },
    { icon: Clock, color: "#82212D", bg: "#fff0f2", value: `${stats?.totalStudyMinutes ?? 0}m`, label: "Time Invested" },
  ];
      ...
      <div className="flex-1 flex items-center justify-center bg-[#F5F2EE]">
        <div className="text-center">
          ...
          <button
            ...
            className="text-maroon-500 hover:underline text-sm"
          >
            Retry
          </button>
        </div>
    ...
    <div className="flex-1 overflow-y-auto bg-[#F5F2EE]">
      ...
      <div className="sticky top-0 z-20 bg-[#F5F2EE]/90 backdrop-blur-sm border-b border-gray-100 flex items-center justify-between px-5 xl:px-8 h-16">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 bg-maroon-50 border border-maroon-200 px-3 py-1.5 rounded-full">
            <Flame size={13} className="text-maroon-500" />
            <span className="text-xs font-bold text-maroon-700">{stats?.streakDays || 0} day streak</span>
          </div>
        </div>
      ...
      <div className="p-5 xl:p-8 max-w-7xl mx-auto">
        ...
        <div className="grid xl:grid-cols-[1fr_420px] gap-5 mb-7">
          ...
          <div className="flex flex-col justify-between">
            <div className="mb-5">
              <p className="text-xs font-semibold uppercase tracking-widest text-maroon-500 mb-1">
                {greeting}, {user?.firstName || "User"} 👋
              </p>
              <h1 className="text-2xl xl:text-3xl font-bold text-gray-900 leading-tight mb-2">
                Your rights are worth<br />
                <span className="text-maroon-500">knowing.</span>
              </h1>
          ...
          <div className="relative rounded-2xl overflow-hidden shadow-lg border border-gray-100 min-h-[240px]"
            ...
            <div className="absolute inset-0 pointer-events-none">
              <div className="absolute top-6 left-6 w-32 h-32 rounded-full bg-maroon-500/10 blur-3xl" />
              ...
              {videoPlaying ? (
                <div className="space-y-1">
                  ...
                  <div className="w-40 h-1 bg-white/10 rounded-full mt-3 mx-auto overflow-hidden">
                    <div className="h-1 bg-maroon-500 rounded-full animate-pulse" style={{ width: "30%" }} />
                  </div>
                </div>
              ) : (
                <>
        ...
        <section className="mb-7">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/dashboard/learn" className="text-xs font-semibold text-maroon-500 hover:underline flex items-center gap-1">
          ...
          {readingItems.length > 0 ? (
            ...
            <div className="text-center py-8 text-gray-500 bg-white rounded-2xl border border-gray-100">
              ...
              <Link href="/dashboard/learn" className="text-maroon-500 hover:underline text-sm mt-2 inline-block">
                Browse topics →
              </Link>
            </div>
          )}
        ...
        <div className="grid xl:grid-cols-3 gap-5 mb-7">
          ...
          <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
            <div className="flex items-center gap-2 mb-4">
              <TrendingUp size={14} className="text-maroon-500" />
                ...
                trending.map((t, i) => (
                  <Link key={i} href={`/dashboard/learn/${t.slug}`}
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-gray-900 group-hover:text-maroon-500 transition-colors truncate">{t.title}</p>
          ...
          <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/dashboard/bookmarks" className="text-[10px] text-maroon-500 font-semibold hover:underline">View all</Link>
            </div>
            <div className="flex flex-col h-full">
              ...
              <div className="flex-shrink-0 pt-3 border-t border-gray-100 mt-2">
                <Link
                  ...
                  className="inline-flex items-center gap-2 text-[11px] text-maroon-500 font-semibold hover:gap-3 transition-all duration-200 group"
                >
        ...
        <div className="grid xl:grid-cols-[1fr_300px] gap-5 mb-7">
          ...
          <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <MessageCircle size={14} className="text-maroon-500" />
                <h3 className="font-bold text-gray-900 text-sm">Community Wins</h3>
              </div>
              <Link href="/dashboard/community" className="text-[10px] text-maroon-500 font-semibold hover:underline">Join discussion</Link>
            </div>
                ...
                community.map((c) => (
                  <div key={c.name} className="flex items-start gap-3 p-3 bg-gray-50 rounded-xl">
                    <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0"
                      style={{ background: `linear-gradient(135deg, ${c.color || "#9B2E3D"}, ${c.color || "#9B2E3D"}80)` }}>
                      {c.initials}
                    </div>
                    <div className="flex-1 min-w-0">
          ...
          <div className="bg-gradient-to-br from-[#111827] to-[#0B1120] rounded-2xl p-5 border border-white/8 relative overflow-hidden">
            <div className="absolute -bottom-8 -right-8 w-32 h-32 rounded-full bg-maroon-500/8 blur-3xl pointer-events-none" />

            <div className="flex items-center gap-2 mb-4">
              <Target size={14} className="text-maroon-400" />
              <span className="text-xs font-bold text-maroon-400! uppercase tracking-wider">Your Next Goal</span>
            </div>
              ...
              <>
                <div className="bg-white/5 border border-white/8 rounded-xl p-4 mb-4">
                  <div className="flex items-center gap-2 mb-2">
                    <Trophy size={16} className="text-maroon-400" />
                  ...
                  <div className="flex items-center gap-2">
                    {Array.from({ length: goal.total || 5 }, (_, i) => (
                      <div key={i} className={`flex-1 h-1.5 rounded-full ${i < (goal.completed || 0) ? "bg-maroon-400" : "bg-white/10"}`} />
                    ))}
                  </div>
        ...
        <div
          ...
          style={{ background: "linear-gradient(135deg, #9B2E3D 0%, #82212D 50%, #8f2431 100%)" }}
        >
          ...
          <Link href="/dashboard/learn"
            className="relative flex-shrink-0 flex items-center gap-2 bg-white text-maroon-600 text-sm font-bold px-6 py-3 rounded-full hover:shadow-xl hover:-translate-y-0.5 transition-all">
            Explore Now
            <ArrowRight size={15} />
          </Link>
        </div>
