  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
        <div className="h-1 w-full bg-gradient-to-r from-maroon-500 to-maroon-600" />
        <div className="p-6">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1.5">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${step === 1 ? "bg-maroon-500 text-white" : "bg-[#111827] text-white"}`}>
                      {step > 1 ? <Check size={11} /> : "1"}
                    </div>
                    <div className="w-5 h-px bg-[#E5E7EB]" />
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${step === 2 ? "bg-maroon-500 text-white" : "bg-[#F3F4F6] text-[#9CA3AF]"}`}>
                      2
                    </div>
                  </div>
                  <button onClick={onClose} className="text-[#9CA3AF] hover:text-[#111827] transition-colors">
                ...
                <div className="space-y-4">
                      ...
                      {CONSULT_MODES.map(m => {
                        ...
                        return (
                          <button key={m.id} onClick={() => setMode(m.id)}
                            className={`flex items-center gap-3 p-4 rounded-xl border-[1.5px] text-left transition-all ${active ? "border-maroon-500 bg-pink-50/60" : "border-[#E5E7EB] hover:border-[#9CA3AF]"}`}>
                            <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${active ? "bg-maroon-500/10" : "bg-[#F3F4F6]"}`}>
                              <Icon size={16} className={active ? "text-maroon-500" : "text-[#6B7280]"} />
                            </div>
                            <div className="flex-1">
                              <p className={`text-[13px] font-semibold ${active ? "text-maroon-500" : "text-[#111827]"}`}>{m.label}</p>
                  ...
                  <div>
                    ...
                    <input value={topic} onChange={e => setTopic(e.target.value)}
                      ...
                      className="w-full h-11 px-4 rounded-xl border-[1.5px] border-[#E5E7EB] text-[13px] text-[#111827] outline-none focus:border-maroon-500 placeholder:text-[#D1D5DB] transition-colors"
                    />
                  </div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {timeSlots.map(s => (
                            <button key={s} onClick={() => setSlot(s)}
                              className={`py-2.5 px-3 rounded-xl border-[1.5px] text-[11px] font-medium text-left transition-all ${slot === s ? "border-maroon-500 bg-pink-50/60 text-maroon-500" : "border-[#E5E7EB] text-[#374151] hover:border-[#9CA3AF]"}`}>
                              {s}
                            </button>
                          ))}
                        </div>
        ...
        <div className="sticky top-0 z-20 bg-[#F5F2EE]/90 backdrop-blur-sm border-b border-gray-200/60 px-5 xl:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setBookmarked(!bookmarked)}
              className={`p-2 rounded-xl border transition-colors ${bookmarked ? "border-maroon-500 bg-pink-50 text-maroon-500" : "border-[#E5E7EB] bg-white text-[#6B7280] hover:border-[#9CA3AF]"}`}>
              <Bookmark size={15} className={bookmarked ? "fill-maroon-500" : ""} />
            </button>
          </div>
        </div>
              ...
              <div className="flex flex-col lg:flex-row gap-6">
                ...
                <div className="lg:w-64 flex flex-col gap-2 flex-shrink-0">
                  ...
                  <button onClick={() => setBookingOpen(true)} disabled={!lawyer.isAvailable}
                    ...
                    style={{ background: lawyer.isAvailable ? "linear-gradient(135deg, #9B2E3D, #82212D)" : "#9CA3AF" }}>
                    <Calendar size={14} />
          ...
          <div className="grid xl:grid-cols-[1fr_280px] gap-5">
                ...
                <div className="flex flex-col gap-3">
                  {[
                    ...
                    <details key={i} className="group bg-white rounded-2xl border border-[#F3F4F6] shadow-sm overflow-hidden">
                      <summary className="flex items-center justify-between gap-3 px-5 py-4 cursor-pointer list-none text-[13px] font-semibold text-[#111827] hover:text-maroon-500 transition-colors">
                        {faq.q}
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="bg-gradient-to-br from-[#111827] to-[#1E3A5F] rounded-2xl p-5 border border-white/6 relative overflow-hidden">
                <div className="absolute -top-6 -right-6 w-20 h-20 rounded-full bg-maroon-500/10 blur-2xl" />
              ...
              <div className="bg-white rounded-2xl border border-[#F3F4F6] shadow-sm p-5">
                ...
                <button onClick={() => setShareOpen(true)}
                  className="w-full py-2.5 rounded-xl border border-[#E5E7EB] text-[12px] font-semibold text-[#6B7280] hover:border-maroon-500 hover:text-maroon-500 flex items-center justify-center gap-1.5 transition-all">
