      ...
      <div className="flex-1 overflow-y-auto bg-[#F5F2EE]">
        ...
        <div className="max-w-7xl mx-auto px-5 xl:px-8 py-7">
          ...
          <div className="rounded-2xl overflow-hidden mb-7 relative"
            ...
            <div className="absolute top-0 left-1/4 w-64 h-64 rounded-full bg-[#7C3AED]/8 blur-[80px] pointer-events-none" />
            ...
            <div className="relative grid md:grid-cols-[1fr_auto] gap-6 p-6 xl:p-8 items-center">
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs font-bold text-[#7C3AED] uppercase tracking-widest">Verified Lawyers Only</span>
                </div>
            ...
            <div className="hidden xl:flex flex-col gap-5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full h-11 pl-9 pr-4 rounded-xl border-[1.5px] border-gray-200 bg-white text-sm text-gray-900 outline-none focus:border-[#7C3AED] placeholder:text-gray-400 transition-colors"
                />
              </div>
                  ...
                  {SPECIALISMS.map(s => {
                    ...
                    return (
                      <button key={s.id} onClick={() => setFilterSpecialism(s.id)}
                        className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm text-left transition-all ${active ? "bg-pink-50 text-[#7C3AED] font-semibold" : "text-gray-600 hover:bg-gray-50"}`}>
                        {/* <Icon size={14} className={active ? "text-[#7C3AED]" : "text-gray-400"} /> */}
                        <span className="flex-1">{s.displayName}</span>
                        <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded-md ${active ? "bg-pink-100 text-[#7C3AED]" : "bg-gray-100 text-gray-400"}`}>{count}</span>
                      </button>
                    );
                  })}
                </div>
                  ...
                  {states.map(s => {
                    ...
                    return (
                      <button key={s} onClick={() => setFilterState(s)}
                        className={`flex items-center justify-between px-3 py-2 rounded-xl text-sm text-left transition-all ${active ? "bg-pink-50 text-[#7C3AED] font-semibold" : "text-gray-600 hover:bg-gray-50"}`}>
              ...
              <div className="bg-[#0B1120] rounded-2xl p-5 border border-white/6">
                <div className="w-9 h-9 rounded-xl bg-[#7C3AED]/15 border border-[#7C3AED]/20 flex items-center justify-center mb-3">
              ...
              <div className="flex items-center gap-3 mb-5">
                ...
                <div className="flex items-center gap-2 ml-auto">
                  ...
                  <select
                    ...
                    className="h-9 px-3 rounded-xl border-[1.5px] border-gray-200 bg-white text-xs text-gray-700 outline-none focus:border-[#7C3AED] transition-colors font-medium"
                  >
              ...
              {filtersOpen && (
                <div className="xl:hidden bg-white rounded-2xl p-4 border border-gray-100 shadow-sm mb-5">
                  <div className="flex flex-wrap gap-2">
                    {SPECIALISMS.map(s => (
                      <button key={s.id} onClick={() => setFilterSpecialism(s.id)}
                        className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${filterSpecialism === s.id ? "bg-[#7C3AED] text-white border-[#7C3AED]" : "border-gray-200 text-gray-600 hover:border-gray-400"}`}>
                        {s.label}
                      </button>
                    ))}
                  </div>
              ...
              {LAWYERS.length === 0 && !isLoading ? (
                <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
                  ...
                  <button onClick={() => { setFilterSpecialism("all"); setFilterState("all"); setSearchQuery(""); }}
                    className="mt-3 text-xs text-[#7C3AED] font-semibold hover:underline">
                    Clear filters
                  </button>
                </div>
              ) : (
                <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-5">
                  ...
                  {/* Request card */}
                  <div
                    className="rounded-2xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center p-8 text-center cursor-pointer hover:border-[#7C3AED]/40 hover:bg-pink-50/30 transition-all group"
                    onClick={() => setModal("request")}
                  >
                    <div className="w-10 h-10 rounded-full bg-gray-100 group-hover:bg-pink-100 flex items-center justify-center mb-3 transition-colors">
                      <Plus size={18} className="text-gray-400 group-hover:text-[#7C3AED] transition-colors" />
                    </div>
