  ...
  return (
    <div className="bg-[#0B1120] rounded-2xl p-5 border border-white/6">
      <div className="flex items-center gap-2 mb-4">
        <Shield size={14} className="text-maroon-500" />
      ...
      <div className="grid grid-cols-5 gap-2">
        {steps.map((s, i) => (
          <div key={s.n} className="relative flex flex-col items-center text-center">
            ...
            <div className="w-8 h-8 rounded-full bg-maroon-500/15 border border-maroon-500/30 flex items-center justify-center mb-2 relative z-10">
              <span className="text-[10px] font-bold text-maroon-500">{s.n}</span>
            </div>
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        <div className="flex items-start gap-3.5 mb-4">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-bold text-gray-900 text-[15px] leading-tight truncate group-hover:text-maroon-500 transition-colors">
              {lawyer.fullName}
            </h3>
          ...
          <button
            ...
            style={{
              background: lawyer.isAvailable
                ? `linear-gradient(135deg, #9B2E3D, #82212D)`
                : "#9CA3AF",
            }}
          >
            ...
            <div className="text-center py-6">
              ...
              <div className="bg-gray-50 rounded-xl p-4 text-left mb-5">
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-bold text-maroon-500">NGN {fee.toLocaleString()}</span>
                </div>
              </div>
            ...
            <>
              ...
              <div className="flex items-center gap-3 mb-5">
                ...
                <div className="ml-auto flex items-center gap-2">
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${step === 1 ? "bg-maroon-500 text-white" : "bg-gray-100 text-gray-400"}`}>1</span>
                  <div className="w-4 h-px bg-gray-200" />
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${step === 2 ? "bg-maroon-500 text-white" : "bg-gray-100 text-gray-400"}`}>2</span>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea value={detail} onChange={e => setDetail(e.target.value)}
                      ...
                      className="w-full h-24 px-4 py-3 rounded-xl border-[1.5px] border-gray-200 text-sm text-gray-900 resize-none outline-none focus:border-maroon-500 placeholder:text-gray-400 transition-colors"
                    />
                  </div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {slots.map(s => (
                          <button key={s} onClick={() => setSlot(s)}
                            className={`py-2.5 px-3 rounded-xl border-[1.5px] text-xs font-medium transition-all text-left ${slot === s ? "border-maroon-500 bg-pink-50/60 text-maroon-500" : "border-gray-200 text-gray-700 hover:border-gray-300"}`}>
                            {s}
                          </button>
                        ))}
                      </div>
                  ...
                  <div className="bg-gray-50 rounded-xl p-4">
                    ...
                    <div className="space-y-1.5 text-xs">
                      ...
                      <div className="flex justify-between border-t border-gray-200 pt-1.5 mt-1.5">
                        ...
                        <span className="font-bold text-maroon-500 text-sm">NGN {fee.toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg relative overflow-hidden max-h-[90vh] overflow-y-auto">
        <div className="h-1 w-full" style={{ background: "linear-gradient(90deg, #0B1120, #9B2E3D)" }} />
        <div className="p-6">
          <button onClick={onClose} className="absolute top-5 right-5 w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center hover:bg-gray-200 transition-colors z-10">
            <X size={14} />
          </button>
                ...
                <div className="flex items-center gap-2">
                  {[1, 2].map(n => (
                    <React.Fragment key={n}>
                      <span className={`w-6 h-6 rounded-full text-[10px] font-bold flex items-center justify-center ${step === n ? "bg-maroon-500 text-white" : step > n ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-400"}`}>
                      ...
                      {SPECIALISMS.filter(s => s._id !== "all").map(s => {
                        ...
                        return (
                          <button key={s._id} onClick={() => setSpecialism(s._id)}
                            className={`flex items-center gap-2 p-3 rounded-xl border-[1.5px] text-left text-xs font-medium transition-all ${specialism === s._id ? "border-maroon-500 bg-pink-50/60 text-maroon-500" : "border-gray-200 text-gray-700 hover:border-gray-300"}`}>
                            <div className={`w-2 h-2 rounded-full ${specialism === s._id ? "bg-maroon-500" : "bg-white"}`} />
                            {s.displayName}
                          </button>
                        );
                      })}
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {urgencies.map(u => (
                        <button key={u.label} onClick={() => setUrgency(u.value as any)}
                          className={`py-2.5 px-3 rounded-xl border-[1.5px] text-xs font-medium transition-all ${urgency === u.value ? "border-maroon-500 bg-pink-50/60 text-maroon-500" : "border-gray-200 text-gray-700 hover:border-gray-300"}`}>
                          {u.label}
                        </button>
                      ))}
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {["Message", "Call", "Video"].map(b => (
                        <button key={b} onClick={() => setMode(b.toLowerCase())}
                          className={`py-2.5 px-3 rounded-xl border-[1.5px] text-xs font-medium transition-all ${mode === b.toLowerCase() ? "border-maroon-500 bg-pink-50/60 text-maroon-500" : "border-gray-200 text-gray-700 hover:border-gray-300"}`}>
                          {b}
                        </button>
                      ))}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {budgets.map(b => (
                        <button key={b} onClick={() => setBudget(b)}
                          className={`py-2.5 px-3 rounded-xl border-[1.5px] text-xs font-medium transition-all ${budget === b ? "border-maroon-500 bg-pink-50/60 text-maroon-500" : "border-gray-200 text-gray-700 hover:border-gray-300"}`}>
                          {b}
                        </button>
                      ))}
                    </div>
