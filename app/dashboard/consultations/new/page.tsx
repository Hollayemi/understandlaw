          ...
          <div key={label} className="flex items-center flex-1 last:flex-none">
            <div className="flex flex-col items-center gap-1.5">
              <div
                ...
                style={current ? { background: "linear-gradient(135deg, #E8317A, #ff6fa8)" } : undefined}
              >
  ...
  return (
    <div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <button
          onClick={() => onPick("firm")}
          className="group text-left bg-white rounded-2xl border-[1.5px] border-[#F3F4F6] hover:border-[#E8317A] p-6 transition-all hover:-translate-y-0.5 hover:shadow-lg relative overflow-hidden"
        >
          <span className="absolute top-4 right-4 text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#FFF0F5] text-[#E8317A] border border-[#FBCFE8]">
            Recommended
          </span>
          <div
          ...
          <div className="mt-4 flex items-center gap-1 text-xs font-bold text-[#E8317A]">
    ...
    return (
      <div className="bg-white rounded-2xl border border-[#F3F4F6] p-8 text-center max-w-xl mx-auto">
        <div className="w-14 h-14 rounded-full bg-[#FFF0F5] border-2 border-[#FBCFE8] flex items-center justify-center mx-auto mb-4">
          <ClipboardList size={26} className="text-[#E8317A]" />
        </div>
                ...
                {SPECIALISMS.map((s: any) => (
                  <button
                    ...
                    className={`flex items-center gap-2 p-3 rounded-xl border-[1.5px] text-left text-xs font-medium transition-all ${specialism === s._id
                      ? "border-[#E8317A] bg-pink-50/60 text-[#E8317A]"
                      : "border-gray-200 text-gray-700 hover:border-gray-300"
                      }`}
                  >
                    <div className={`w-2 h-2 rounded-full shrink-0 ${specialism === s._id ? "bg-[#E8317A]" : "bg-gray-200"}`} />
                    {s.displayName}
                  </button>
                ))}
              </div>
          ...
          <div className="space-y-5">
            <div>
              ...
              <textarea
                ...
                className="w-full h-28 px-4 py-3 rounded-xl border-[1.5px] border-gray-200 text-sm resize-none outline-none focus:border-[#E8317A] placeholder:text-gray-400 transition-colors"
              />
            </div>
          ...
          <div className="space-y-5">
            ...
            <label className="flex items-start gap-2.5 cursor-pointer">
              <input
                ...
                className="mt-0.5 accent-[#E8317A] w-3.5 h-3.5 shrink-0"
              />
...
function FileDropzone({ onFiles }: { onFiles: (files: FileList | null) => void }) {
  ...
  return (
    <div
      ...
      className={`rounded-xl border-2 border-dashed p-6 text-center cursor-pointer transition-all ${dragging ? "border-[#E8317A] bg-pink-50/50" : "border-gray-200 hover:border-gray-300 bg-gray-50/60"
        }`}
    >
      <input
        ref={inputRef}
          ...
          <div className="bg-gray-50 rounded-xl p-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex justify-between pt-2 mt-1.5 border-t border-gray-200">
                ...
                <span className="font-bold text-[#E8317A] text-sm">NGN {chosen.fees[mode].toLocaleString()}</span>
              </div>
            </div>
          </div>
...
function ChosenLawyerBanner({ lawyer, onChange }: { lawyer: LawyerFull; onChange: () => void }) {
  return (
    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
      ...
      <button onClick={onChange} className="text-[11px] font-semibold text-[#E8317A] hover:underline shrink-0">
        Change
      </button>
    </div>
  );
    ...
    <div>
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-16">
          <Loader2 size={24} className="animate-spin text-[#E8317A]" />
        </div>
      ) : LAWYERS.length === 0 ? (
...
function DirectLawyerCard({ lawyer, onSelect }: { lawyer: LawyerFull; onSelect: () => void }) {
  return (
    <button
      ...
      className="text-left bg-white rounded-2xl border-[1.5px] border-gray-100 hover:border-[#E8317A] p-4 transition-all hover:-translate-y-0.5 disabled:opacity-50 disabled:hover:translate-y-0 disabled:hover:border-gray-100 disabled:cursor-not-allowed"
    >
      ...
      <div className="flex items-center justify-between text-[11px]">
        ...
        <span className="font-bold text-[#E8317A] flex items-center gap-0.5">
