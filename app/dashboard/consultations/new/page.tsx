          ...
          <div key={label} className="flex items-center flex-1 last:flex-none">
            <div className="flex flex-col items-center gap-1.5">
              <div
                ...
                style={current ? { background: "linear-gradient(135deg, #F97316, #EA580C)" } : undefined}
              >
  ...
  return (
    <div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <button
          onClick={() => onPick("firm")}
          className="group text-left bg-white rounded-2xl border-[1.5px] border-[#F3F4F6] hover:border-[#F97316] p-6 transition-all hover:-translate-y-0.5 hover:shadow-lg relative overflow-hidden"
        >
          <span className="absolute top-4 right-4 text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#FFF0F5] text-[#F97316] border border-[#FBCFE8]">
            Recommended
          </span>
          <div
    ...
    return (
      <div className="bg-white rounded-2xl border border-[#F3F4F6] p-8 text-center max-w-xl mx-auto">
        <div className="w-14 h-14 rounded-full bg-[#FFF0F5] border-2 border-[#FBCFE8] flex items-center justify-center mx-auto mb-4">
          <ClipboardList size={26} className="text-[#F97316]" />
        </div>
                ...
                {SPECIALISMS.map((s: any) => (
                  <button
                    ...
                    className={`flex items-center gap-2 p-3 rounded-xl border-[1.5px] text-left text-xs font-medium transition-all ${specialism === s._id
                      ? "border-[#F97316] bg-pink-50/60 text-[#F97316]"
                      : "border-gray-200 text-gray-700 hover:border-gray-300"
                      }`}
                  >
                    <div className={`w-2 h-2 rounded-full shrink-0 ${specialism === s._id ? "bg-[#F97316]" : "bg-gray-200"}`} />
                    {s.displayName}
                  </button>
                ))}
              </div>
          ...
          <div className="space-y-5">
            <div>
              ...
              <textarea
                ...
                className="w-full h-28 px-4 py-3 rounded-xl border-[1.5px] border-gray-200 text-sm resize-none outline-none focus:border-[#F97316] placeholder:text-gray-400 transition-colors"
              />
            </div>
            ...
            <div>
              <SectionLabel>Preferred Communication Mode</SectionLabel>
               <p className="text-[11px] text-gray-400 -mt-1.5 mb-4">
                Your selected communication preference will <span className="font-bold text-[#F97316]! ">expire in 2 hours</span>.
              </p>
                ...
                {CONSULT_MODES.map((m, i) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center gap-1.5 py-3 px-2 rounded-xl border-[1.5px] transition-all ${mode === m.id ? "border-[#F97316] bg-pink-50/60" : "border-gray-200 hover:border-gray-300"
                        }`}
                    >
                      <Icon size={15} className={mode === m.id ? "text-[#F97316]" : "text-gray-500"} />
                      <span className={`text-[11px] font-semibold ${mode === m.id ? "text-[#F97316]" : "text-gray-700"}`}>{m.label}</span>
                      <span className={`text-[11px] font-semibold ${mode === m.id ? "text-[#F97316]" : "text-gray-700"}`}>NGN {1000 * (i + 1)}</span>
                    </button>
                  );
                })}
              </div>
            ...
            {waiver && (
              <div className="mt-3">
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 text-sm border-[1.5px] border-gray-200 rounded-xl focus:border-[#F97316] focus:ring-2 focus:ring-pink-100 outline-none transition-all resize-none"
                  rows={3}
                  required
                />
              </div>
          ...
          <div className="space-y-5">
            ...
            <label className="flex items-start gap-2.5 cursor-pointer">
              <input
                ...
                className="mt-0.5 accent-[#F97316] w-3.5 h-3.5 shrink-0"
              />
...
function FileDropzone({ onFiles }: { onFiles: (files: FileList | null) => void }) {
  ...
  return (
    <div
      ...
      className={`rounded-xl border-2 border-dashed p-6 text-center cursor-pointer transition-all ${dragging ? "border-[#F97316] bg-pink-50/50" : "border-gray-200 hover:border-gray-300 bg-gray-50/60"
        }`}
    >
      <input
        ref={inputRef}
          ...
          <div className="bg-gray-50 rounded-xl p-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex justify-between pt-2 mt-1.5 border-t border-gray-200">
                ...
                <span className="font-bold text-[#F97316] text-sm">NGN {chosen.fees[mode].toLocaleString()}</span>
              </div>
            </div>
          </div>
...
function ChosenLawyerBanner({ lawyer, onChange }: { lawyer: LawyerFull; onChange: () => void }) {
  return (
    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
      ...
      <button onClick={onChange} className="text-[11px] font-semibold text-[#F97316] hover:underline shrink-0">
        Change
      </button>
    </div>
  );
    ...
    <div>
      <div className="flex items-start gap-3 p-4 bg-white border border-[#F3F4F6] rounded-2xl mb-5">
        <div className="w-8 h-8 rounded-xl bg-[#FFF0F5] flex items-center justify-center shrink-0">
          <BadgeCheck size={13} className="text-[#F97316]" />
        </div>
        <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full h-10 pl-9 pr-4 rounded-xl border-[1.5px] border-gray-200 bg-white text-sm outline-none focus:border-[#F97316] placeholder:text-gray-400 transition-colors"
          />
        </div>
        <div>
          <select value={location} onChange={e => setLocation(e.target.value)}
            className="h-10 px-3 rounded-xl border-[1.5px] border-gray-200 bg-white text-xs text-gray-700 outline-none focus:border-[#F97316] transition-colors font-medium"
          >
            <option value="">Any State</option>
            {NIGERIAN_STATES.map((e: any, i: number) => <option key={i} value={e.code}>{e.label}</option>)}
          </select>
...
function DirectLawyerCard({ lawyer, onSelect }: { lawyer: LawyerFull; onSelect: () => void }) {
  return (
    <button
      ...
      className="text-left bg-white rounded-2xl border-[1.5px] border-gray-100 hover:border-[#F97316] p-4 transition-all hover:-translate-y-0.5 disabled:opacity-50 disabled:hover:translate-y-0 disabled:hover:border-gray-100 disabled:cursor-not-allowed"
    >
      ...
      <div className="flex items-center justify-between text-[11px]">
        ...
        <span className="font-bold text-[#F97316] flex items-center gap-0.5">
