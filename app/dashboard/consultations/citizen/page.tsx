...
const REQUEST_STATUS_CFG: Record<MatchStatus, { label: string; bg: string; text: string; dot: string }> = {
  ...
  recommended: { label: "Lawyers Ready", bg: "#FFF0F5", text: "#9D174D", dot: "#F97316" },
      ...
      <div className="flex-1 overflow-y-auto bg-[#F5F2EE]">
        ...
        <div className="max-w-6xl mx-auto px-5 xl:px-8 py-7">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-1 bg-white border border-[#F3F4F6] rounded-xl p-1 w-fit">
              {([
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg text-[13px] font-bold transition-all ${mainTab === opt.v ? "bg-[#111827] text-white" : "text-[#6B7280] hover:text-[#111827]"
                    }`}
                >
                  {opt.l}
                  {!!opt.count && (
                    <span
                      className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${mainTab === opt.v ? "bg-white/20 text-white" : "bg-[#FFF0F5] text-[#F97316]"
                        }`}
                    >
                      {opt.count}
                    </span>
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6 ">
                {[
                  ...
                  { label: "Awaiting", value: uiStats.waiting, color: "#F97316", bg: "#FFF0F5", icon: Clock, pulse: uiStats.waiting > 0 },
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {s.pulse && <span className="w-2 h-2 rounded-full bg-[#F97316] animate-pulse" />}
                      </div>
                  ...
                  {([
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-[11px] font-semibold transition-all whitespace-nowrap ${tab === opt.v ? "bg-white text-[#111827] shadow-sm" : "text-[#6B7280] hover:text-[#111827]"
                          }`}
                      >
                        {opt.l}
                        {count > 0 && (
                          <span
                            className={`text-[10px] px-1 py-0.5 rounded-full font-bold ${tab === opt.v ? "bg-[#F97316] text-white" : "bg-[#F3F4F6] text-[#9CA3AF]"
                              }`}
                          >
                            {count}
                          </span>
...
function RequestsTab({
  ...
  return (
    <div>
      ...
      {/* Filter row */}
      <div className="flex items-center  gap-1 bg-[#F9FAFB] border border-[#F3F4F6] rounded-xl p-1 mb-5 overflow-x-auto w-full">
        {([
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-[11px] font-semibold transition-all whitespace-nowrap ${requestTab === opt.v ? "bg-white text-[#111827] shadow-sm" : "text-[#6B7280] hover:text-[#111827]"
              }`}
          >
            {opt.l}
          </button>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-col-reverse gap-2 flex-wrap">
                    <p className="text-[13px] font-bold text-gray-900">{r.topic}</p>
                  ...
                  <p className="text-[11px] text-gray-500 mt-0.5">
                    {r.status === "recommended"
                      ...
                      : r.status === "matched"
                        ? `Matched with ${r.matchedLawyerId ? r.matchedLawyerId.firstName : "your lawyer"}`
                        : r.status === "in_review"
