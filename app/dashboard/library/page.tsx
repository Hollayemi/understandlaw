...
const CATEGORY_CONFIG: Record<BookCategory, { label: string; color: string; bg: string; icon: React.ElementType }> = {
  ...
  consumer: { label: "Consumer Rights", color: "#7C3AED", bg: "#FFF0F5", icon: Globe },
  road: { label: "Road Traffic", color: "#7C3AED", bg: "#FFF7ED", icon: Car },
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5">
          <div className="flex items-center gap-3 p-3 bg-[#F9FAFB] rounded-xl border border-[#F3F4F6] mb-5">
            ...
            <div className="text-right flex-shrink-0">
              <p className="text-[14px] font-bold text-[#7C3AED]">NGN {(order.totalAmount || 0).toLocaleString()}</p>
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden max-h-[92vh] flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5">
          <div className="flex items-center gap-3 p-3 bg-[#F9FAFB] rounded-xl border border-[#F3F4F6] mb-5">
            ...
            <div className="text-right flex-shrink-0">
              <p className="text-[14px] font-bold text-[#7C3AED]">NGN {(book.pricePhysical || 0).toLocaleString()}</p>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea value={form.notes} onChange={set("notes")} placeholder="Any delivery instructions..."
                  className="w-full h-16 px-3 py-2.5 rounded-xl border-[1.5px] border-[#E5E7EB] text-[13px] text-[#111827] resize-none outline-none focus:border-[#7C3AED] placeholder:text-[#D1D5DB] transition-colors"
                />
              </div>
              ...
              <div className="bg-[#F9FAFB] rounded-xl border border-[#F3F4F6] p-4">
                ...
                <div className="space-y-2 text-[12px]">
                  ...
                  <div className="flex justify-between border-t border-[#E5E7EB] pt-2">
                    ...
                    <span className="font-bold text-[#7C3AED] text-[14px]">NGN {total.toLocaleString()}</span>
                  </div>
                </div>
              </div>
    ...
    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 flex flex-col">
      <div className="relative h-44 flex-shrink-0" style={{ background: `linear-gradient(135deg, ${cat.color}20, ${cat.color}08)` }}>
        ...
        <button onClick={() => setSaved(!saved)}
          ...
          <Heart size={14} className={saved ? "text-[#7C3AED] fill-[#7C3AED]" : "text-gray-500"} />
        </button>
      </div>
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className="flex items-center gap-3 text-[10px] text-gray-400 mb-3">
          ...
          {book.format !== "pdf" && (
            <span className="flex items-center gap-1">
              <ShoppingCart size={10} className="text-[#7C3AED]" />
        ...
        {book.pricePhysical !== null && (
          <p className="text-[13px] font-bold text-[#7C3AED] mb-3">
        ...
        <div className="max-w-7xl mx-auto px-5 xl:px-8 py-7">
              ...
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-5">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs font-bold text-[#7C3AED] uppercase tracking-widest">Legal Library</span>
                  </div>
            ...
            <section className="mb-7">
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-bold text-gray-900 text-base flex items-center gap-2">
                  <span className="w-1.5 h-4 rounded-full bg-[#7C3AED] inline-block" />
                  Featured Books
                </h2>
              </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-6">
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <div>
                <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className={`w-full h-10 px-4 rounded-xl border-[1.5px] border-gray-200 text-sm outline-none focus:border-[#7C3AED] placeholder:text-gray-400 transition-colors bg-white`}>
          ...
          {books.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
              ...
              <button onClick={() => { setSearch(""); setCategoryFilter("all"); setFormatFilter("all"); }}
                className="mt-3 text-xs text-[#7C3AED] font-semibold hover:underline">Clear filters</button>
            </div>
          ) : (
