    ...
    <HomeWrapper>
      ...
      <section className="bg-white pt-14 pb-14 border-b border-gray-100">
        <div className="max-w-4xl mx-auto px-6 lg:px-8 text-center">
          <p className="text-xs font-semibold uppercase tracking-widest text-maroon-500 mb-3">Pricing</p>
          <h1
            className="text-[clamp(48px,7vw,80px)] leading-none tracking-[0.01em] text-gray-900 uppercase mb-4"
            style={{ fontFamily: "var(--font-bebas)" }}
          >
            START FREE.<br />
            <span style={{ color: "#9B2E3D" }}>SCALE WHEN READY.</span>
          </h1>
            ...
            {TIERS.map((tier) => (
              ...
              >
                {tier.badge && (
                  <div
                    className={`absolute -top-px left-0 right-0 h-1 ${
                      tier.highlight ? "bg-gradient-to-r from-maroon-500 to-maroon-600" : "bg-gradient-to-r from-gray-200 to-gray-300"
                    }`}
                  />
                )}
                {tier.badge && (
                  <div className={`absolute top-3 right-4 text-[10px] font-bold px-2.5 py-1 rounded-full ${
                    tier.highlight ? "bg-maroon-500 text-white" : "bg-gray-100 text-gray-500"
                  }`}>
                    {tier.badge}
                  </div>
                )}
                ...
                <div className="p-7 flex flex-col flex-1">
                  <p className={`text-xs font-bold uppercase tracking-widest mb-2 ${tier.highlight ? "text-maroon-500" : "text-gray-400"}`}>
                    {tier.name}
                  </p>
                  ...
                  <ul className="flex flex-col gap-2.5 mb-7 flex-1">
                    {tier.features.map((f) => (
                      <li key={f.text} className="flex items-start gap-2.5">
                        {f.included ? (
                          <svg className="w-4 h-4 flex-shrink-0 mt-0.5 text-maroon-500" fill="currentColor" viewBox="0 0 20 20">
                  ...
                  <Link
                    ...
                    className={`w-full py-3 rounded-full text-sm font-bold flex items-center justify-center gap-2 transition-all ${
                      tier.highlight
                        ? "bg-maroon-500 text-white hover:bg-maroon-600 hover:shadow-lg hover:shadow-pink-900/30"
      ...
      <section className="bg-white py-16 xl:py-20 border-t border-gray-100">
        <div className="max-w-6xl mx-auto px-6 lg:px-8">
          <div className="flex items-end justify-between gap-6 mb-8">
            <div>
              <p className="text-xs font-semibold uppercase tracking-widest text-maroon-500 mb-2">For Nigerian Lawyers</p>
              <h2
                ...
                <ul className="flex flex-col gap-2 mb-5">
                  {tier.features.map((f) => (
                    <li key={f} className="flex items-center gap-2 text-xs text-gray-600">
                      <svg className="w-3.5 h-3.5 flex-shrink-0 text-maroon-500" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                      </svg>
                      {f}
                    </li>
      ...
      <section className="bg-[#F3F3F3] py-16 xl:py-20">
        <div className="max-w-3xl mx-auto px-6 lg:px-8">
          <div className="text-center mb-10">
            <p className="text-xs font-semibold uppercase tracking-widest text-maroon-500 mb-2">FAQ</p>
            <h2
              className="text-[clamp(28px,4vw,44px)] leading-none text-gray-900 uppercase"
              style={{ fontFamily: "var(--font-bebas)" }}
            >
          ...
          <div className="flex flex-col gap-3">
            {FAQS.map((faq, i) => (
              <details key={i} className="group bg-white border border-gray-100 rounded-2xl overflow-hidden">
                <summary className="flex items-center justify-between gap-4 px-6 py-5 cursor-pointer list-none text-sm font-semibold text-gray-900 hover:text-maroon-500 transition-colors">
                  {faq.q}
