...
export default function AdminLibraryPage() {
  ...
  const orderColumns = [
    ...
    },
    {
      ...
      header: "Amount",
      render: (o: BookOrder) => (
        <span className="text-[12px] font-bold text-maroon-500">NGN {o.totalAmount.toLocaleString()}</span>
      ),
    },
    {
      key: "status",
  ...
  return (
    <>
      ...
      <div className="p-6 xl:p-8 max-w-7xl mx-auto">
        ...
        {/* Stats */}
        <StatBar items={[
          { label: "Total Books", value: stats?.totalBooks || 0, icon: BookOpen, color: "#9B2E3D", bg: "#FFF0F5" },
          ...
          {([
            ...
            return (
              <button key={t.id} onClick={() => setActiveTab(t.id)}
                ...
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${activeTab === t.id ? "bg-maroon-500 text-white" : "bg-[#F3F4F6] text-[#9CA3AF]"}`}>{t.count}</span>
              </button>
            );
          })}
        </div>
          ...
          <>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-5">
              <div className="relative flex-1 max-w-xs">
                ...
                <input 
                  ...
                  className="w-full h-9 pl-9 pr-4 rounded-xl border-[1.5px] border-[#E5E7EB] text-[13px] outline-none focus:border-maroon-500 placeholder:text-[#D1D5DB] transition-colors"
                />
              </div>
              ...
              {(booksFetching || booksLoading) && (
                <Loader2 size={16} className="animate-spin text-maroon-500 ml-2" />
              )}
            </div>
            ...
            {books.length === 0 && !booksFetching && !booksLoading ? (
              <div className="bg-white rounded-2xl border border-[#F3F4F6] p-16 text-center">
                ...
                <button onClick={() => setShowUpload(true)}
                  className="mt-4 flex items-center gap-1.5 mx-auto text-[12px] font-bold text-maroon-500 hover:underline">
              ...
              <>
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                  ...
                  {/* Upload card */}
                  <button onClick={() => setShowUpload(true)}
                    className="rounded-2xl border-2 border-dashed border-[#E5E7EB] flex flex-col items-center justify-center p-8 text-center hover:border-maroon-500/40 hover:bg-pink-50/20 transition-all group min-h-[340px]">
