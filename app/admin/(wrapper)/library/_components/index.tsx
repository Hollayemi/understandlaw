...
export function UploadBookModal({ onClose, onAdd }: { onClose: () => void; onAdd: (b: Book) => void }) {
  ...
  const textareaCls = "w-full px-3 py-2.5 rounded-xl border-[1.5px] border-[#E5E7EB] text-[13px] text-[#111827] resize-none outline-none focus:border-maroon-500 placeholder:text-[#D1D5DB] transition-colors";
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-[11px] font-bold text-[#6B7280] uppercase tracking-wider mb-1.5">
                  Book Title <span className="text-maroon-500">*</span>
                </label>
                <input
                  value={form.title}
                  onChange={set("title")}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] font-bold text-[#6B7280] uppercase tracking-wider mb-1.5">
                    Author <span className="text-maroon-500">*</span>
                  </label>
                  <input
                    value={form.author}
                    onChange={set("author")}
              ...
              <div>
                <label className="block text-[11px] font-bold text-[#6B7280] uppercase tracking-wider mb-1.5">
                  Description <span className="text-maroon-500">*</span>
                </label>
                <textarea
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block text-[11px] font-bold text-[#6B7280] uppercase tracking-wider mb-1.5">
                    ISBN <span className="text-maroon-500">*</span>
                  </label>
                  <input
                    value={form.isbn}
                    onChange={set("isbn")}
                ...
                <div>
                  <label className="block text-[11px] font-bold text-[#6B7280] uppercase tracking-wider mb-1.5">
                    Year <span className="text-maroon-500">*</span>
                  </label>
                  <input
                ...
                <div>
                  <label className="block text-[11px] font-bold text-[#6B7280] uppercase tracking-wider mb-1.5">
                    Pages <span className="text-maroon-500">*</span>
                  </label>
                  <input
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-[11px] font-bold text-[#6B7280] uppercase tracking-wider mb-2">
                  Book Format <span className="text-maroon-500">*</span>
                </label>
                <div className="grid grid-cols-3 gap-2">
                  {(["pdf", "physical", "both"] as BookFormat[]).map(f => (
                    <button
                      ...
                      className={`py-3 rounded-xl border-[1.5px] text-[12px] font-semibold transition-all ${form.format === f ? "border-maroon-500 bg-pink-50 text-maroon-500" : "border-[#E5E7EB] text-[#6B7280] hover:border-[#9CA3AF]"}`}
                    >
              ...
              {form.format !== "physical" && (
                <div>
                  <label className="block text-[11px] font-bold text-[#6B7280] uppercase tracking-wider mb-2">
                    PDF File <span className="text-maroon-500">*</span>
                  </label>
                  <div className="flex gap-2 mb-2">
                    {(["upload", "url"] as const).map(t => (
                      <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-[11px] font-semibold transition-all ${form.pdfType === t ? "border-maroon-500 bg-pink-50 text-maroon-500" : "border-[#E5E7EB] text-[#6B7280]"}`}
                      >
                    ...
                    <>
                      <ThumbnailUpload images={pdfs} title=" " type="pdf" setImages={setPDFs} maxImages={1}>
                        {pdfs.length > 0 ? <div>
                          ...
                          className="border-2 w-full border-dashed border-[#E5E7EB] rounded-xl p-5 text-center hover:border-maroon-500 transition-colors cursor-pointer"
                        >
                ...
                <div className="flex gap-2 mb-2">
                  {(["upload", "url"] as const).map(t => (
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-[11px] font-semibold transition-all ${form.coverType === t ? "border-maroon-500 bg-pink-50 text-maroon-500" : "border-[#E5E7EB] text-[#6B7280]"}`}
                    >
                      {t === "upload" ? <Upload size={11} /> : <Globe size={11} />}
              ...
              {form.format !== "pdf" && (
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[11px] font-bold text-[#6B7280] uppercase tracking-wider mb-1.5">
                      Price (NGN) <span className="text-maroon-500">*</span>
                    </label>
                    <input
              ...
              {isUploading && (
                <div className="space-y-2">
                  ...
                  <div className="h-1.5 bg-[#F3F4F6] rounded-full overflow-hidden">
                    <div
                      className="h-1.5 bg-maroon-500 rounded-full transition-all duration-200"
              ...
              <button
                ...
                className="w-full py-2.5 rounded-xl text-[13px] font-bold text-white bg-maroon-500 hover:bg-maroon-600 disabled:opacity-60 flex items-center justify-center gap-2 transition-colors"
              >
                {isUploading ? (
                  <>
        ...
        <div className="p-6">
          ...
          <div className="flex items-center gap-3 p-3 bg-[#F9FAFB] rounded-xl border border-[#F3F4F6] mb-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[12px] font-bold text-maroon-500 mt-0.5">NGN {order.totalAmount.toLocaleString()}</p>
            </div>
          </div>
          ...
          {(order.status === "processing" || order.status === "shipped") && (
            <div className="mb-4">
              ...
              <input value={tracking} onChange={e => setTracking(e.target.value)}
                ...
                className="w-full h-10 px-3 rounded-xl border-[1.5px] border-[#E5E7EB] text-[13px] text-[#111827] outline-none focus:border-maroon-500 placeholder:text-[#D1D5DB] transition-colors"
              />
            </div>
          )}
          ...
          <div className="flex flex-col gap-2">
            ...
            {order.status === "processing" && (
              <button onClick={() => update("shipped")} disabled={saving}
                className="w-full py-2.5 rounded-xl text-[12px] font-bold text-white bg-[#8B5CF6] hover:bg-maroon-500 flex items-center justify-center gap-1.5 disabled:opacity-60 transition-colors">
    ...
    <div className="bg-white rounded-2xl border border-[#F3F4F6] overflow-hidden hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 flex flex-col">
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className="grid grid-cols-2 gap-2 mb-3 bg-[#F9FAFB] rounded-xl p-2.5">
          ...
          {book.format !== "pdf" && (
            <div className="text-center">
              <p className="text-[12px] font-bold text-maroon-500">{book.orderCount}</p>
        ...
        {book.pricePhysical !== null && (
          <div className="flex items-center gap-1.5 text-[12px] font-bold text-maroon-500 mb-3">
            <DollarSign size={11} />
        ...
        <div className="flex gap-2 mt-auto">
          ...
          <button onClick={() => onEdit(book)}
            className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl border border-[#E5E7EB] text-[11px] font-semibold text-[#6B7280] hover:border-maroon-500 hover:text-maroon-500 transition-all">
            <Edit2 size={11} /> Edit
          </button>
        </div>
      </div>
