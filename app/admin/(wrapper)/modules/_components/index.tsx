...
export const CATEGORY_CONFIG: Record<ModuleCategory, { label: string; icon: React.ElementType; color: string; bg: string }> = {
  ...
  consumer: { label: "Consumer Rights", icon: Globe, color: "#9B2E3D", bg: "#FFF0F5" },
  road: { label: "Road Traffic", icon: Car, color: "#9B2E3D", bg: "#FFF7ED" },
};
...
export function CreateModuleModal({ onClose }: { onClose: () => void }) {
  ...
  const inputCls = "w-full h-11 px-4 rounded-xl border-[1.5px] border-[#E5E7EB] text-[13px] text-[#111827] outline-none focus:border-maroon-500 placeholder:text-[#D1D5DB] transition-colors";
          ...
          <div className="flex items-center gap-2 mb-5">
            {[1, 2].map(n => (
              <React.Fragment key={n}>
                <div className={`flex items-center justify-center w-6 h-6 rounded-full text-[10px] font-bold transition-all ${step === n ? "bg-maroon-500 text-white" : step > n ? "bg-[#111827] text-white" : "bg-[#F3F4F6] text-[#9CA3AF]"
                  }`}>{n}</div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select value={form.category} onChange={set("category")}
                  className="w-full h-11 px-4 rounded-xl border-[1.5px] border-[#E5E7EB] text-[13px] text-[#111827] outline-none focus:border-maroon-500 transition-colors bg-white">
              ...
              <div>
                ...
                <textarea value={form.description} onChange={set("description")}
                  ...
                  className="w-full h-20 px-4 py-3 rounded-xl border-[1.5px] border-[#E5E7EB] text-[13px] text-[#111827] resize-none outline-none focus:border-maroon-500 placeholder:text-[#D1D5DB] transition-colors"
                />
              </div>
              <div className="">
                ...
                <select value={form.instructorId} onChange={set("instructorId")}
                  className="h-9 px-3 rounded-xl border-[1.5px] border-[#E5E7EB] w-full text-[12px] text-[#6B7280] bg-white outline-none focus:border-maroon-500 transition-colors">
...
export function ModuleCard({ mod, onDelete, onEdit }: { mod: Module; onDelete: (id: string) => void; onEdit: (mod: Module) => void }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#F3F4F6] overflow-hidden hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 flex flex-col">
      <div className="relative h-36 flex-shrink-0" style={{ background: `linear-gradient(135deg, ${cat?.color || '#9B2E3D'}20, ${cat?.color || '#9B2E3D'}08)` }}>
        {mod.thumbnail ? (
          <img src={mod.thumbnail} alt={mod.title} className="w-full h-full object-cover" />
        ) : (
          <div className="w-full h-full flex items-center justify-center">
            <CatIcon size={40} style={{ color: cat?.color || '#9B2E3D', opacity: 0.25 }} />
          </div>
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent" />
        <div className="absolute top-3 left-3 flex items-center gap-1.5">
          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full"
            style={{ background: cat?.bg || '#FFF0F5', color: cat?.color || '#9B2E3D' }}>
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <Link href={`/admin/modules/${mod.id}`}
          className="mt-3 flex items-center justify-center gap-1.5 py-2 rounded-xl border border-[#E5E7EB] text-[12px] font-semibold text-[#6B7280] hover:border-maroon-500 hover:text-maroon-500 transition-all">
        ...
        <div className="p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea value={form.description} onChange={set("description")}
                className="w-full h-20 px-4 py-3 rounded-xl border-[1.5px] border-[#E5E7EB] text-[13px] text-[#111827] resize-none outline-none focus:border-maroon-500"
              />
            </div>
            <div>
