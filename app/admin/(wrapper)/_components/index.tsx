...
export function StatCard({ icon: Icon, label, value, trend, color = "#9B2E3D", bg = "#FFF0F5" }: StatCardProps) {
...
export function Avatar({ initials, color = "#9B2E3D", size = "md" }: AvatarProps) {
  return (
    <div
          ...
          >
            ...
            {opt.count !== undefined && (
              <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${
                value === opt.value ? "bg-maroon-500 text-white" : "bg-[#F3F4F6] text-[#9CA3AF]"
              }`}>
                {opt.count}
              </span>
            )}
