...
import { useAdminCreatePlanMutation } from "@/redux/slices/admin/subscription.slice";
import { showSuccess, showError } from "@/app/components/ui/sonner";

export default function CreatePlanPage() {
  const router = useRouter();
  const [createPlan, { isLoading }] = useAdminCreatePlanMutation();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showError(error?.data?.message || "Failed to create plan");
    }
  };
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Define a new subscription plan for lawyers
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full h-11 px-4 rounded-lg border border-gray-200 text-sm outline-none focus:border-[#E8317A] transition-colors"
            />
            {errors.name && (
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full px-4 py-3 rounded-lg border border-gray-200 text-sm outline-none focus:border-[#E8317A] transition-colors resize-none"
            />
            {errors.description && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full h-11 px-4 rounded-lg border border-gray-200 text-sm outline-none focus:border-[#E8317A] transition-colors bg-white"
              >
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={addFeature}
              className="flex items-center gap-1.5 text-sm text-[#E8317A] hover:text-[#d02a6e] font-medium"
            >
              <Plus size={16} />
              Add Feature
            </button>
            ...
            {form.features.map((feature, index) => (
              <div key={index} className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 h-11 px-4 rounded-lg border border-gray-200 text-sm outline-none focus:border-[#E8317A] transition-colors"
                />
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="flex items-center gap-2 px-6 py-2.5 rounded-lg bg-[#E8317A] text-white text-sm font-semibold hover:bg-[#d02a6e] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isLoading ? (
